...
def K_FACTOR(elo_diff, underdog_won, tries):
	...
	if underdog_won: 	
		return ((elo_diff + ACR_Globals.__MAX_ELO) / ACR_Globals.__MAX_ELO) * (tries / ACR_Globals.__MAX_TRIES) * 0.5
	else:				
		return (1 - elo_diff / ACR_Globals.__MAX_ELO) * (tries / ACR_Globals.__MAX_TRIES) * 0.5
